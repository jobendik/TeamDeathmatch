...
export function updateHardpoint(dt: number): void {
  ...
  if (player && player.hp > 0 && player.renderComponent) {
    const dx = player.renderComponent.position.x - active.position.x;
    const dz = player.renderComponent.position.z - active.position.z;
    if (dx * dx + dz * dz <= radiusSq) {
      if (player.team === TEAM_BLUE) state.playersInPoint.blue++;
      else if (player.team === TEAM_RED) state.playersInPoint.red++;
    }
  }
  const agents = gameState.agents ?? [];
  for (const a of agents) {
    if (!a || a.hp <= 0 || !a.renderComponent) continue;
    const dx = a.renderComponent.position.x - active.position.x;
    const dz = a.renderComponent.position.z - active.position.z;
    if (dx * dx + dz * dz <= radiusSq) {
      if (a.team === TEAM_BLUE) state.playersInPoint.blue++;
      else if (a.team === TEAM_RED) state.playersInPoint.red++;
    }
  }
