...
import { getLoadouts, getActiveLoadout, setActiveLoadout, updateLoadout,
  PERKS, FIELD_UPGRADES, LETHALS, TACTICALS, isPerkUnlocked } from '@/loadout/Loadouts';
import { getContracts, claimAllCompleted, getActiveContractCount } from '@/contracts/ContractSystem';
...
function renderCareerTab(): string {
  ...
  const pctToNext = xp.neededForNext > 0 ? (xp.intoLevel / xp.neededForNext) * 100 : 100;
  const careerKD = getOverallKD().toFixed(2);
  const careerWR = (getWinRate() * 100).toFixed(1);
  const hoursPlayed = ((p.career.timePlayedSec ?? 0) / 3600).toFixed(1);
  const prestigeLevel = p.prestige ?? 0;

  return `
    <div class="mm-career">
      <div class="mm-career-head">
        <div class="mm-avatar">${p.displayName.substring(0, 2).toUpperCase()}</div>
        <div class="mm-career-name">
          <div class="mm-career-username">${p.displayName}</div>
          <div class="mm-career-level">
            <span class="mm-level-pill">${prestigeLevel > 0 ? `P${prestigeLevel}` : ''} LVL ${p.level}</span>
            <span class="mm-level-sub">${xp.intoLevel.toLocaleString()} / ${xp.neededForNext.toLocaleString()} XP</span>
          </div>
      ...
      <div class="mm-stats-grid">
        <div class="mm-stat"><div class="mm-stat-label">KILLS</div><div class="mm-stat-val">${(p.career.kills ?? 0).toLocaleString()}</div></div>
        <div class="mm-stat"><div class="mm-stat-label">DEATHS</div><div class="mm-stat-val">${(p.career.deaths ?? 0).toLocaleString()}</div></div>
        ...
        <div class="mm-stat"><div class="mm-stat-label">HEADSHOTS</div><div class="mm-stat-val">${(p.career.headshots ?? 0).toLocaleString()}</div></div>
        <div class="mm-stat"><div class="mm-stat-label">LONGEST SHOT</div><div class="mm-stat-val">${Math.round(p.career.longestShot ?? 0)}m</div></div>
        ...
        <div class="mm-stat"><div class="mm-stat-label">WINS</div><div class="mm-stat-val">${p.career.wins ?? 0}</div></div>
        <div class="mm-stat"><div class="mm-stat-label">LOGIN STREAK</div><div class="mm-stat-val">${p.dailyStreak ?? 0}d</div></div>
      </div>

      <div class="mm-section-head">WEAPON MASTERY</div>
      <div class="mm-weapons">
        ${Object.entries(p.weapons ?? {}).slice(0, 8).map(([wpn, ws]: [string, any]) => `
...
function renderLoadoutTab(): string {
  ...
  const editing = loadouts.loadouts[state.editingLoadoutIndex];
  if (!editing) return '<div class="mm-empty">No loadouts</div>';

  const perkRow = (slot: 'perk1' | 'perk2' | 'perk3', perkGroup: number) => {
    const selectedId = editing.perks[perkGroup];
    const availablePerks = Object.values(PERKS).filter(p => p.slot === perkGroup + 1);
    return `
      <div class="mm-slot" data-slot="${slot}">
        <div class="mm-slot-label">PERK ${perkGroup + 1}</div>
        <div class="mm-slot-val">${PERKS[selectedId]?.name ?? '—'}</div>
        <div class="mm-slot-desc">${PERKS[selectedId]?.description ?? ''}</div>
      </div>
    `;
  };
  ...
  return `
    <div class="mm-loadouts-list">
      ${loadouts.loadouts.map((l, i) => `
        <div class="mm-loadout-btn ${i === state.editingLoadoutIndex ? 'active' : ''} ${i === loadouts.activeIndex ? 'equipped' : ''}" data-loadout-idx="${i}">
          <div class="mm-loadout-name">${l.name}</div>
          <div class="mm-loadout-weapons">${l.primary} · ${l.secondary}</div>
          ${i === loadouts.activeIndex ? '<div class="mm-equipped-badge">EQUIPPED</div>' : ''}
        </div>
      `).join('')}
    </div>
    ...
    <div class="mm-loadout-editor">
      ...
      <div class="mm-slots-grid">
        <div class="mm-slot" data-slot="lethal">
          ...
          <div class="mm-slot-val">${LETHALS[editing.lethal]?.name ?? '—'}</div>
        </div>
        <div class="mm-slot" data-slot="tactical">
          <div class="mm-slot-label">TACTICAL</div>
          <div class="mm-slot-val">${TACTICALS[editing.tactical]?.name ?? '—'}</div>
        </div>
        <div class="mm-slot" data-slot="field">
          <div class="mm-slot-label">FIELD UPGRADE</div>
          <div class="mm-slot-val">${FIELD_UPGRADES[editing.fieldUpgrade]?.name ?? '—'}</div>
        </div>
      </div>
...
function renderContractsTab(): string {
  ...
  const contractRow = (c: any) => {
    ...
    return `
      <div class="mm-contract ${c.completed ? 'completed' : ''} ${c.claimed ? 'claimed' : ''}">
        <div class="mm-contract-head">
          <div class="mm-contract-title">${c.name}</div>
          <div class="mm-contract-reward">+${c.xpReward} XP</div>
        </div>
        <div class="mm-contract-desc">${c.description}</div>
        <div class="mm-contract-progress">
          <div class="mm-contract-bar"><div class="mm-contract-fill" style="width:${Math.min(100, pct)}%"></div></div>
          <div class="mm-contract-num">${c.progress} / ${c.goal}</div>
        </div>
        ${c.completed && !c.claimed ? '<div class="mm-contract-claim">READY TO CLAIM</div>' : ''}
      </div>
    `;
  };
  ...
  return `
    <div class="mm-contract-head-bar">
      <div class="mm-contract-title-main">ACTIVE CONTRACTS (${getActiveContractCount()})</div>
    ...
    <div class="mm-cos-grid">
      <div class="mm-cos-section">
        ...
        <div class="mm-cos-row">
          <div class="mm-cos-label">Blue Team</div>
          <div class="mm-cos-val">${eq.operatorBlue}</div>
        </div>
        <div class="mm-cos-row">
          <div class="mm-cos-label">Red Team</div>
          <div class="mm-cos-val">${eq.operatorRed}</div>
        </div>
      </div>
      ...
      <div class="mm-cos-section">
        ...
        <div class="mm-cos-emotes">
          ${eq.emotes.slice(0, 4).map((e: string, i: number) => `
      ...
      <div class="mm-cos-section">
        ...
        <div class="mm-cos-emotes">
          ${eq.sprays.slice(0, 3).map((s: string, i: number) => `
            <div class="mm-cos-slot">
              <div class="mm-cos-slot-num">${i + 1}</div>
      ...
      <div class="mm-cos-section">
        ...
        <div class="mm-cos-val">${eq.finisher ?? 'default'}</div>
      </div>

      <div class="mm-cos-section">
        <div class="mm-section-head">INTRO</div>
        <div class="mm-cos-val">${eq.intro ?? 'default'}</div>
      </div>
      ...
      <div class="mm-cos-section mm-cos-unlock">
        ...
        <div class="mm-cos-unlocks">
          ...
          <div>${(unlocked.weaponSkins ?? []).length} weapon skins</div>
        </div>
      </div>
    </div>
  `;
