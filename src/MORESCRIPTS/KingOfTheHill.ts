...
function updateKothVisual(): void {
  ...
  const ud = (state.zone.mesh as any).userData;
...
export function updateKoth(dt: number): void {
  ...
  if (player && player.health > 0 && player.mesh) {
    const dx = player.mesh.position.x - state.zone.position.x;
    const dz = player.mesh.position.z - state.zone.position.z;
    if (dx * dx + dz * dz <= rSq) {
      if (player.team === 'blue') state.playersInZone.blue++;
      else if (player.team === 'red') state.playersInZone.red++;
    }
  }
  const agents = (gameState as any).agents ?? [];
  for (const a of agents) {
    if (!a || a.health <= 0 || !a.mesh) continue;
    const dx = a.mesh.position.x - state.zone.position.x;
    const dz = a.mesh.position.z - state.zone.position.z;
    if (dx * dx + dz * dz <= rSq) {
      if (a.team === 'blue') state.playersInZone.blue++;
      else if (a.team === 'red') state.playersInZone.red++;
    }
  }
