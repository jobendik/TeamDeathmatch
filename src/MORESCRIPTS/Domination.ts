...
function makeZoneVisual(zone: DomZone, scene: THREE.Scene): THREE.Object3D {
  ...
  group.add(fill);
  (group as any).userData.fillMat = fillMat;
  ...
  pillar.position.y = 2;
  group.add(pillar);
  (group as any).userData.pillarMat = pillarMat;
...
function updateZoneColor(zone: DomZone, visual: THREE.Object3D): void {
  const userData = (visual as any).userData;
...
export function initDomination(
  ...
  createDomHud();
  announce('DOMINATION', { sub: 'Capture A, B, C to score', tier: 'high', duration: 3 });
  return state;
}
...
function getAllCombatants(): Array<{ team: Team; pos: THREE.Vector3; alive: boolean }> {
  ...
  if (player && player.health > 0 && player.mesh) {
    out.push({ team: player.team as Team, pos: player.mesh.position, alive: true });
  }
  const agents = (gameState as any).agents ?? [];
  for (const a of agents) {
    if (!a || a.health <= 0 || !a.mesh) continue;
    out.push({ team: a.team as Team, pos: a.mesh.position, alive: true });
  }
  return out;
}
