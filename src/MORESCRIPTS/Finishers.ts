...
import { awardAccountXP, profileMutate } from '@/progression/PlayerProfile';
...
export function findFinisherTarget(): any | null {
  ...
  for (const agent of agents) {
    ...
    const dist = agent.mesh.position.distanceTo(pPos);
    if (dist > bestDist) continue;
    if (!isTargetInBackCone(player.mesh, agent.mesh)) continue;
...
export function tryInitiateFinisher(): boolean {
  ...
  finisherState.targetStartPos.copy(target.mesh.position);
  finisherState.targetStartQuat.copy(target.mesh.quaternion);
  finisherState.attackerStartPos.copy(player.mesh.position);
  finisherState.attackerStartPitch = (gameState as any).camera?.rotation.x ?? 0;
  finisherState.attackerStartYaw = (gameState as any).camera?.rotation.y ?? 0;
  finisherState.cameraBase.copy((gameState as any).camera?.position ?? player.mesh.position);
  ...
  cachedTimeScale = (gameState as any).timeScale ?? 1;
  (gameState as any).timeScale = TIME_SCALE_DURING;

  // Lock player movement
  (gameState as any)._finisherLockMovement = true;
...
export function updateFinisher(dt: number): void {
  ...
  finisherState.t += dt / Math.max(0.1, (gameState as any).timeScale ?? 1);

  const t = finisherState.t;
  const player = gameState.player;
  const target = finisherState.target;
  const camera = (gameState as any).camera as THREE.Camera | undefined;

  if (!player?.mesh || !target?.mesh) {
    endFinisher();
    return;
  }
  ...
  if (t < 0.3) {
    ...
    const targetFwd = new THREE.Vector3(0, 0, -1).applyQuaternion(target.mesh.quaternion);
    const behindOffset = targetFwd.multiplyScalar(-1.1);
    const desiredPos = target.mesh.position.clone().add(behindOffset);
    player.mesh.position.lerp(desiredPos, 0.25);

    // Face attacker toward target
    const toTarget = target.mesh.position.clone().sub(player.mesh.position);
  ...
  if (t >= 0.3 && t < 2.1) {
    if (camera) {
      ...
      const desiredCamPos = target.mesh.position.clone().add(camOffset);
      camera.position.lerp(desiredCamPos, 0.15);
      camera.lookAt(target.mesh.position.x, target.mesh.position.y + 1.2, target.mesh.position.z);
    }
  }
...
function endFinisher(): void {
  // Restore
  (gameState as any).timeScale = cachedTimeScale;
  (gameState as any)._finisherLockMovement = false;
