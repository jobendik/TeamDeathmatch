...
function raycastForPing(camera: THREE.Camera): {
  ...
  state.raycaster.setFromCamera({ x: 0, y: 0 }, camera);
  ...
  for (const a of agents) {
    ...
    const toAgent = new THREE.Vector3().subVectors(a.mesh.position, origin);
  ...
  if (bestEnemy) {
    return {
      pos: bestEnemy.mesh.position.clone(),
      kind: 'enemy',
      dist: bestEnemyDist,
    };
  }
  ...
  window.addEventListener('keyup', (e) => {
    ...
    if (state.wheelActive) {
      ...
      if (selected && camera) {
        ...
        placePing(selected, pos, gameState.player?.team ?? null);
      }
      ...
      placePing(kind, hit.pos, gameState.player?.team ?? null);
    }
  });
}
