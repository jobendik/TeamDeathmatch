...
function updateBombVisual(): void {
  ...
  const ud = (state.bombMesh as any).userData;
...
function startRound(): void {
  ...
  if (state.round > 1 && (state.round - 1) === state.config.swapAfterRound) {
    ...
    announce('SIDES SWAPPED', { tier: 'high', duration: 3 });
  }
  ...
  if (gameState.player && gameState.player.revive) {
    try { gameState.player.revive(); } catch { /* */ }
  }
...
function countAliveByTeam(): { blue: number; red: number } {
  ...
  if (player && player.health > 0) {
    if (player.team === 'blue') out.blue++;
    else if (player.team === 'red') out.red++;
  }
  const agents = (gameState as any).agents ?? [];
  for (const a of agents) {
    if (!a || a.health <= 0) continue;
    if (a.team === 'blue') out.blue++;
    else if (a.team === 'red') out.red++;
  }
  return out;
}
...
function findPlantingAttacker(): any | null {
  ...
  if (gameState.player && gameState.player.health > 0 && gameState.player.team === state.attackerTeam) {
    allPotentials.push(gameState.player);
  }
  const agents = (gameState as any).agents ?? [];
  for (const a of agents) {
    if (!a || a.health <= 0) continue;
    if (a.team !== state.attackerTeam) continue;
    allPotentials.push(a);
  }

  for (const p of allPotentials) {
    if (!p.mesh) continue;
    const dx = p.mesh.position.x - state.config.bombSite.x;
    const dz = p.mesh.position.z - state.config.bombSite.z;
...
function findDefusingDefender(): any | null {
  ...
  const pool: any[] = [];
  if (gameState.player && gameState.player.health > 0 && gameState.player.team === defenderTeam) {
    pool.push(gameState.player);
  }
  const agents = (gameState as any).agents ?? [];
  for (const a of agents) {
    if (!a || a.health <= 0) continue;
    if (a.team !== defenderTeam) continue;
    pool.push(a);
  }

  for (const p of pool) {
    if (!p.mesh) continue;
    const dx = p.mesh.position.x - state.bombPosition.x;
    const dz = p.mesh.position.z - state.bombPosition.z;
...
export function updateSd(dt: number): void {
  ...
  if (state.roundPhase === 'prep' && state.roundElapsed >= PREP_TIME) {
    ...
    announce('GO!', { tier: 'high', duration: 1.2, color: '#22d66a' });
    return;
  }
  ...
  if (state.roundPhase === 'live' && !state.bombPlanted) {
    ...
    if (planter) {
      ...
      if (state.plantProgress >= state.config.plantTime) {
        ...
        state.bombPosition = planter.mesh.position.clone();
        state.bombPosition.y = 0;
        state.bombMesh = makeBombVisual(state.scene!, state.bombPosition);
        state.roundPhase = 'planted';
        state.plantProgress = 0;
        state.currentPlanter = null;
        announce('BOMB PLANTED', { sub: `${state.config.bombTimer}s`, tier: 'high', color: '#ff3344', duration: 3 });
        try {
          const s: any = require('@/audio/SoundHooks');
          s.playObjective?.();
        } catch { /* */ }
      }
    } else {
  ...
  if (state.roundPhase === 'planted') {
    ...
    if (defuser) {
      ...
      if (state.defuseProgress >= state.config.defuseTime) {
        announce('BOMB DEFUSED', { tier: 'high', color: '#22d66a', duration: 2.5 });
...
function triggerDetonation(): void {
  ...
  try {
    const s: any = require('@/audio/SoundHooks');
    s.playExplosion?.() ?? s.playThunder?.();
  } catch { /* */ }

  // Damage anyone within blast radius (optional — mainly it's the round end)
  const blastRadius = 8;
  const blastSq = blastRadius * blastRadius;
  const allTargets: any[] = [gameState.player, ...((gameState as any).agents ?? [])].filter(Boolean);
  for (const t of allTargets) {
    if (!t.mesh || t.health <= 0) continue;
    const dx = t.mesh.position.x - state.bombPosition.x;
    const dz = t.mesh.position.z - state.bombPosition.z;
    if (dx * dx + dz * dz <= blastSq) {
      t.health = 0; // bomb kills everyone in radius
    }
  }
}
...
function updateSdHud(): void {
  ...
  if (player && player.health > 0 && player.mesh) {
    const defenderTeam: Team = state.attackerTeam === 'blue' ? 'red' : 'blue';
    const pTeam = player.team as Team;
    const bombSite = state.config.bombSite;
    const dx = player.mesh.position.x - bombSite.x;
    const dz = player.mesh.position.z - bombSite.z;
    ...
    if (state.roundPhase === 'live' && pTeam === state.attackerTeam && atSite && !state.bombPlanted) {
      ...
      const bdx = player.mesh.position.x - state.bombPosition.x;
      const bdz = player.mesh.position.z - state.bombPosition.z;
